...
Graph Neural Networks were chosen over classical ML specifically because the dataset is small (~42 patients). GNNs leverage the relational structure of brain connectivity directly, making them well-suited to settings where flattening data into feature vectors would discard too much spatial information.
...
**`01_gat_longitudinal_pipeline.ipynb`** — v1 (reference, incomplete)

Introduces the core longitudinal feature engineering idea: instead of using a static snapshot of each patient's brain, compute the *change* in combined structural + functional connectivity between consecutive visits, then average across all available visits per patient. This inter-visit delta captures neuroplasticity directly as the predictive signal. The GAT model architecture is also defined here. Edge construction and dataset saving are incomplete in this version — see notebook 02 for the full working pipeline.
...
- **Edge construction:** uses `dense_to_sparse` to properly convert adjacency matrices to sparse graph format
- **Gradient clipping:** `max_norm=1.0` added for training stability
- **Unused `edge_attr` removed** from GAT forward pass
...
|---|---|
| Nodes | Brain regions (~344, standard brain atlas) |
...
Fix: `GroupShuffleSplit(groups=patient_ids)` keeps all visits of a patient entirely within train or entirely within test. The lower accuracy is the real one — and still a meaningful improvement over the 59.65% prior best on this dataset.
...
```bash
git clone https://github.com/YOUR_USERNAME/brain-network-gat.git
cd brain-network-gat
...
- **Adaptive thresholding:** per-patient edge sparsification based on lesion load
...
Code: MIT License  
Data: Not included — proprietary clinical data subject to IRB protocols.
